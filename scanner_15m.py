...
def get_candles(coin, interval="4h", limit=120):
    """ดึงข้อมูลกราฟพร้อมระบบป้องกันแครช"""
    try:
        ...
        print(f"Gate.io {interval} fetch error for {coin}: {e}")
    ...
    try:
        ...
        print(f"KuCoin {interval} fetch error for {coin}: {e}")

    return None

def detect_macd_divergence(df, lookback=90):
    """ตรวจจับ Dual-Mode Divergence"""
    try:
...
def check_4h_trend(df_4h):
    ...
    try:
        ...
        print(f"4H Trend error: {e}")

    return "NONE"

def check_15m_trigger(df_15m, trend_4h):
    """ตรวจจังหวะจบย่อ 15M (Custom MACD 12, 26, SMA 9)"""
    try:
...
def detect_macd_divergence(df, lookback=90):
    """ตรวจจับ Dual-Mode Divergence (Micro/Macro)"""
    try:
        fast_ema = df["close"].ewm(span=12, adjust=False).mean()
        slow_ema = df["close"].ewm(span=26, adjust=False).mean()
        macd = fast_ema - slow_ema
        ...
        # 1. Bullish Divergence (สำหรับฝั่ง SHORT)
        ...
        # 2. Bearish Divergence (สำหรับฝั่ง LONG)
...
def main():
    ...
    for coin in WATCHLIST:
        ...
        # 3. ตรวจสอบ Divergence ทั้ง 4H และ 15M (Dual-Mode)
        div_4h = detect_macd_divergence(df_4h)
        div_15m = detect_macd_divergence(df_15m)
        sym = f"{coin}USDT"
...
def detect_macd_divergence(df, lookback=100):
    """ตรวจจับ Macro/Micro Divergence ทั้งใน 4H และ 15M"""
    try:
        fast_ema = df["close"].ewm(span=12, adjust=False).mean()
        slow_ema = df["close"].ewm(span=26, adjust=False).mean()
        macd = fast_ema - slow_ema
