...
# จัดกลุ่มสินทรัพย์ (ใช้โครงสร้างเดิมจาก [source: 11] 100%)
SECTORS = {
...
# ลดรูปชื่อ Sector สำหรับแสดงผลหลังชื่อเหรียญให้สั้นลง
...
# ==========================================
# ROUTER FETCHING LOGIC (ยึด Core เดิม 100%)
# ==========================================
def get_binance_candles(symbol, timeframe, limit=250):
...
def get_gateio_candles(symbol, timeframe, limit=250):
...
def get_kucoin_candles(symbol, timeframe, limit=250):
    base_sym = symbol[:-4] if symbol.endswith("USDT") else symbol
...
"""
    ...
    if GEMINI_API_KEY:
        try:
            ...
            # บังคับใช้คำสั่งเดี่ยว gemini-3.6-flash เท่านั้น
